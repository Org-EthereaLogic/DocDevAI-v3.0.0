...
# FastAPI app instance with security headers
app = FastAPI(
    title="DevDocAI API",
    ...
    docs_url="/api/docs",
    redoc_url="/api/redoc",
    openapi_url=None if os.getenv("PRODUCTION") else "/api/openapi.json",  # Disable in production
)
...
app.add_middleware(
    ...
    allowed_hosts=["localhost", "127.0.0.1", "localhost:3000", "localhost:8002"],
)
...
def sanitize_input(value: str) -> str:
    ...
    value = re.sub(r"(?i)(javascript|data):", "", value)
    return value.strip()
...
def validate_path(path: str) -> bool:
    ...
    if ".." in str(normalized) or normalized.is_absolute():
        if not str(normalized).startswith(str(Path.cwd())):
            return False
    ...
    for pattern in suspicious_patterns:
        if re.search(pattern, str(normalized), re.IGNORECASE):
            return False
    return True
...
class GenerateDocumentRequest(BaseModel):
    template: constr(regex=r"^[a-zA-Z0-9_-]+$", max_length=50)
    context: Dict[str, Any]
    output_format: constr(regex=r"^(markdown|html|pdf)$") = "markdown"
...
class EnhanceDocumentRequest(BaseModel):
    content: constr(max_length=1000000)  # 1MB limit
    strategy: constr(regex=r"^[A-Z_]+$", max_length=50) = "MIAIR_ENHANCED"
    target_quality: confloat(ge=0.0, le=1.0) = 0.85
...
async def health_check():
    """Health check endpoint for desktop connection status"""
...
async def get_configuration():
    ...
    try:
        ...
        # Get API keys (fully masked for security)
        def mask_key(key):
            if not key or key == "your_key_here":
                return None
            # Never expose any part of the API key
...
async def generate_document(request: Request, doc_request: GenerateDocumentRequest):
...
async def enhance_document(request: Request, doc_request: EnhanceDocumentRequest):
    ...
    try:
        ...
        # EnhancementPipeline only needs config
...
async def analyze_document(request: Request, doc_request: AnalyzeDocumentRequest):
        ...
        if hasattr(analysis, "iterations"):
            try:
                if isinstance(analysis.iterations, int):
                    ...
                    # Try to get length, fallback to int conversion
                    issues_found = len(analysis.iterations)
            except TypeError:
                # If len() fails, try to convert to int or use default
                try:
                    issues_found = int(analysis.iterations)
...
# Desktop-specific filesystem endpoints for local project integration
...
class FileSystemSaveRequest(BaseModel):
    file_path: constr(max_length=500)
    content: constr(max_length=10000000)  # 10MB limit
    ...
    @validator("content")
    def sanitize_content(cls, v):
        # For file content, we don't sanitize as aggressively
...
async def browse_filesystem(request: Request, fs_request: FileSystemBrowseRequest):
    """Browse local filesystem for project directories"""
    try:
        ...
        return {"error": f"Failed to browse filesystem: {str(e)}"}


@app.post("/api/filesystem/read")
@limiter.limit("100 per minute")
async def read_file(request: Request, fs_request: FileSystemReadRequest):
    ...
    try:
        ...
        return {"error": "File is not a text file or has encoding issues"}
    except Exception as e:
        return {"error": f"Failed to read file: {str(e)}"}


@app.post("/api/filesystem/save")
@limiter.limit("50 per minute")
async def save_file(request: Request, fs_request: FileSystemSaveRequest):
    ...
    try:
        ...
        return {
            ...
            "saved": True,
            "backup_created": request.create_backup and file_path.exists(),
        }
    except Exception as e:
