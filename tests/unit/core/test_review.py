...
from devdocai.core.review_strategies import (
    QualityScore,
)

from devdocai.core.review_security import (
    SecurityError,
    ValidationError,
    RateLimitError,
)

from devdocai.core.review_types import (
    ReviewConfig,
    ReviewType,
    ReviewResult,
    SecurityIssue,
...
from devdocai.core.reviewers import (
...
from devdocai.core.tracking import TrackingMatrix, DependencyGraph
...
class TestReviewEngine:
    ...
    async def test_requirements_reviewer(self, review_engine):
        ...
        result = await review_engine.analyze(
            document, 
            review_types=[ReviewType.REQUIREMENTS]
        )
    ...
    async def test_performance_reviewer(self, review_engine):
        ...
        result = await review_engine.analyze(
            document,
            review_types=[ReviewType.PERFORMANCE]
        )
    ...
    async def test_consistency_reviewer_integration(self, review_engine):
        ...
        review_engine.tracking.get_dependencies = Mock(
    ...
    async def test_llm_integration(self, review_engine):
        ...
        # Mock LLM adapter
        llm_adapter = Mock()
        llm_adapter.generate = AsyncMock(
            return_value=Mock(content="Enhanced analysis result")
        )
    ...
    async def test_design_reviewer(self, review_engine):
        ...
        result = await review_engine.analyze(
            document,
            review_types=[ReviewType.DESIGN]
        )
        ...
        assert len(design_review.metrics["components_identified"]) >= 2  # Identified at least some components
    ...
    async def test_usability_reviewer(self, review_engine):
        ...
        result = await review_engine.analyze(
            document,
            review_types=[ReviewType.USABILITY]
        )
    ...
    async def test_test_coverage_reviewer(self, review_engine):
        ...
        result = await review_engine.analyze(
            document,
            review_types=[ReviewType.TEST_COVERAGE]
        )
    ...
    async def test_compliance_reviewer(self, review_engine):
        ...
        result = await review_engine.analyze(
            document,
            review_types=[ReviewType.COMPLIANCE]
        )
...
class TestReviewEnginePerformance:
    ...
    async def test_batch_processing_performance(self, review_engine):
        ...
        print(f"Batch processing: {len(documents)} docs in {elapsed_time:.2f}s (avg: {avg_time:.2f}s)")
