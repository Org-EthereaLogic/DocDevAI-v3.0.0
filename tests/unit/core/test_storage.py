...
class TestStorageManager:
    ...
    def test_encryption_with_unique_iv(self, storage_manager):
        ...
        cursor = storage_manager._conn.cursor()
        cursor.execute("SELECT encrypted_content, iv FROM documents WHERE id IN (?, ?)",
                      ("doc1", "doc2"))
    ...
    def test_hmac_integrity_check(self, storage_manager):
        ...
        doc = Document(
            id="test_doc_006",
            content="Content with integrity",
            type="text"
        )
        storage_manager.save_document(doc)
        ...
        cursor = storage_manager._conn.cursor()
        cursor.execute("SELECT encrypted_content FROM documents WHERE id = ?",
                      ("test_doc_006",))
        encrypted = cursor.fetchone()[0]
        
        # Modify encrypted content
        tampered = encrypted[:-10] + b'tampered!!'
        cursor.execute("UPDATE documents SET encrypted_content = ? WHERE id = ?",
                      (tampered, "test_doc_006"))
    ...
    def test_bulk_operations(self, storage_manager):
        ...
        docs = [
            Document(id=f"bulk_{i}", content=f"Content {i}", type="text")
            for i in range(10)
        ]
