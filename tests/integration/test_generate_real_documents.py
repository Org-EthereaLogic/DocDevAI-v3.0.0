...
async def generate_readme_document():
    ...
    try:
        ...
        if result.success and result.document:
            ...
            print_status("success", "README generated successfully!")
...
async def generate_api_documentation():
    ...
    try:
        ...
        # Generate document
        result = await generator.generate_document(
            template_name="api_doc", context=context, use_cache=False, validate=True
        )
        ...
        if result.success and result.document:
            ...
            print_status("success", "API documentation generated successfully!")
            print(f"  • Generation time: {generation_time:.2f} seconds")
            print(f"  • Document size: {len(result.document):,} characters")
            print(f"  • Saved to: {output_file}")
...
async def generate_changelog():
    ...
    try:
        ...
        # Generate document
        result = await generator.generate_document(
            template_name="changelog", context=context, use_cache=False, validate=True
        )
        ...
        if result.success and result.document:
            ...
            print_status("success", "Changelog generated successfully!")
            print(f"  • Generation time: {generation_time:.2f} seconds")
            print(f"  • Document size: {len(result.document):,} characters")
            print(f"  • Saved to: {output_file}")
...
async def check_audit_logs():
    ...
    if audit_file.exists():
        ...
        recent_logs = [
            line for line in lines[-20:] if "llm_generation" in line or "api_call" in line
        ]
