...
async def test_llm_adapter(config: ConfigurationManager) -> Tuple[bool, Optional[LLMAdapter]]:
    ...
    try:
        ...
        response = await adapter.generate(
            prompt="Say exactly: 'DevDocAI Backend Test Successful'", max_tokens=30
        )
        ...
        if response:
            ...
            print_metric(
                "Response",
                response.strip()[:100] + "..." if len(response) > 100 else response.strip(),
            )
        ...
        for template_config in templates:
            ...
            try:
                ...
                if result.success and result.document:
                    ...
                    results[template_name] = {"success": False, "error": result.error}
...
async def test_storage_integration(config: ConfigurationManager) -> bool:
    ...
    try:
        ...
        test_doc = {
            ...
            "metadata": {"created": datetime.now().isoformat(), "type": "test"},
        }

        # Store document
        doc_id = storage.store_document(
            doc_type="test", content=test_doc["content"], metadata=test_doc["metadata"]
        )
...
async def test_failover_mechanism(config: ConfigurationManager) -> bool:
    ...
    try:
        ...
        _ = config.get_api_key(config.llm.provider)  # noqa: F841

        # Try generation with potentially failing providers
        response = await adapter.generate(
            prompt="Test failover: respond with 'Failover successful'", max_tokens=30
        )
...
async def test_performance_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        context = {"project_name": "Performance Test", "description": "Testing cache performance"}
...
async def main():
    ...
    doc_successes = sum(
        1
        for r in test_results["document_generation"].values()
        if isinstance(r, dict) and r.get("success")
    )
    total_docs = len(test_results["document_generation"])

    all_passed = test_results["configuration"] and test_results["llm_adapter"] and doc_successes > 0
    ...
    if all_passed:
        ...
        print(
            f"{Colors.GREEN}Generated {doc_successes}/{total_docs} documents successfully{Colors.ENDC}"
        )
        print(
            f"\n{Colors.CYAN}📁 Generated documents saved in: ./generated_documents/{Colors.ENDC}"
        )
        print(
            f"{Colors.CYAN}Please review the generated documents for quality assessment{Colors.ENDC}"
        )
    else:
