...
class TestResourceLimits:
    ...
    async def test_document_size_limit(self):
        ...
        with pytest.raises(ResourceLimitError):
            SuiteConfig(
                suite_id="test_suite",
                documents=[{"id": "doc1", "content": large_content}]
            )
    ...
    async def test_cross_reference_limit(self):
        ...
        refs = {f"doc_{i}": [f"ref_{j}" for j in range(100)] 
                for i in range(MAX_CROSS_REFS // 100 + 1)}
...
class TestHMACValidation:
    ...
    async def test_consistency_report_hmac(self):
        ...
        assert manager._verify_hmac(
            report_data,
            report.details["integrity_hmac"]
        )
