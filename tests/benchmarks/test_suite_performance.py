...
class TestSuitePerformance:
    ...
    async def original_manager(self, mock_dependencies):
        ...
        config, storage, generator, tracking = mock_dependencies
        return SuiteManager(config=config, storage=storage, generator=generator, tracking=tracking)
    ...
    async def test_suite_generation_performance(self, optimized_manager):
        ...
        suite_config = SuiteConfig(
            ...
            documents=[
                {"id": f"doc_{i}", "type": "markdown", "template": "default"} for i in range(10)
            ],
            cross_references={"doc_0": ["doc_1", "doc_2"], "doc_1": ["doc_3", "doc_4"]},
        )
    ...
    async def test_consistency_analysis_performance(self, optimized_manager):
        ...
        optimized_manager.storage.get_document = AsyncMock(
            side_effect=lambda doc_id: next((doc for doc in documents if doc.id == doc_id), None)
        )
    ...
    async def test_concurrent_operations(self, optimized_manager):
        ...
        suite_configs = [
            SuiteConfig(
                ...
                documents=[{"id": f"suite_{i}_doc_{j}", "type": "markdown"} for j in range(5)],
            )
            for i in range(50)
        ]
        ...
        start_time = time.time()
        tasks = [optimized_manager.generate_suite(config) for config in suite_configs]
    ...
    async def test_memory_mode_adaptation(self, mock_dependencies):
        ...
        for mode in ["minimal", "balanced", "performance", "maximum"]:
            ...
            documents = [
                Document(id=f"doc_{i}", content=f"Content {i}", type="markdown") for i in range(50)
            ]

            manager.storage.list_documents = AsyncMock(return_value=[doc.id for doc in documents])
    ...
    async def test_performance_vs_original(self, optimized_manager, original_manager):
        ...
        suite_config = SuiteConfig(
            ...
            documents=[{"id": f"doc_{i}", "type": "markdown"} for i in range(20)],
        )
    ...
    async def test_memory_usage(self, optimized_manager):
        ...
        for i in range(5):
            suite_config = SuiteConfig(
                ...
                documents=[{"id": f"suite_{i}_doc_{j}", "type": "markdown"} for j in range(100)],
            )
...
if __name__ == "__main__":
    ...
    pytest.main([__file__, "-v", "-s", "-m", "benchmark", "--tb=short"])
