...
from devdocai.core.generator import (
    DocumentGenerator, ResponseCache, ContextCache, 
    BatchProcessor, BatchRequest, PerformanceMonitor
)
...
class TestParallelProcessing:
    ...
    async def test_batch_processor_performance(self, config_performance, sample_project):
        ...
        # Validate results
        assert len(results) == batch_size, f"Not all documents generated: {len(results)}/{batch_size}"
...
class TestMemoryModeScaling:
    ...
    async def test_memory_mode_performance(self, temp_dir, sample_project):
        ...
        perf_speedup = results['performance']['docs_per_second'] / results['baseline']['docs_per_second']
...
class TestPerformanceBenchmarks:
    ...
    async def test_sustained_throughput(self, generator_performance, sample_project):
        ...
        assert cache_stats['hit_rate'] > 0.5, f"Cache hit rate too low: {cache_stats['hit_rate']:.2%}"
...
async def test_performance_report(generator_performance, sample_project):
    ...
    for scenario, metrics in results.items():
        print(f"{scenario:<15} {metrics['count']:<10} "
