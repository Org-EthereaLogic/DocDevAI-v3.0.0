...
class TestStreamingExporter:
    ...
    async def test_stream_json_export(self, tmp_path):
        ...
        data = {
            "packages": [
                {"name": f"package_{i}", "version": f"1.0.{i}"}
                for i in range(1000)
            ]
        }
...
class TestPerformanceIntegration:
    ...
    def test_parallel_caching_integration(self):
        ...
        results = optimizer.parallel_processor.process_batch(
            items, process_item, chunk_size=10
        )
    ...
    async def test_async_batch_integration(self, tmp_path):
        ...
        files_data = [
            (tmp_path / f"file_{i}.json", {"id": i, "data": f"content_{i}"})
            for i in range(10)
        ]
        
        # Write files asynchronously
        tasks = [
            AsyncFileHandler.write_json_async(path, data)
