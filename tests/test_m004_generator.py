...
class TestDocumentGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_document_success(self, mock_config, mock_llm_adapter, mock_storage, sample_template, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_generate_document_validation_failure(self, mock_config, mock_llm_adapter, mock_storage):
        ...
        generator.template_manager.load_template = Mock(return_value={
            'document_type': 'readme',
            'sections': [
                {'name': 'header', 'prompt_template': 'Generate header'}
            ]
        })
    ...
    async def test_generate_with_retry(self, mock_config, mock_llm_adapter, mock_storage):
        ...
        result = await generator.generate('readme', '/test/project', retry_on_failure=True, max_retries=2)
        
        assert result['quality_score'] == 88
        assert generator.validator.validate.call_count == 2
    
    @pytest.mark.asyncio
    async def test_generate_sections_parallel(self, mock_config, mock_llm_adapter, mock_storage, sample_template):
    ...
    def test_template_loading_from_disk(self, tmp_path):
        ...
        template_content = {
            ...
            'sections': [
                {
                    'name': 'intro',
                    'prompt_template': 'Generate introduction for {project_name}'
                }
            ]
        }
        ...
        # Override config
        config.get = lambda key, default=None: str(template_dir) if key == 'templates.dir' else default
...
class TestEdgeCases:
    ...
    async def test_concurrent_generation(self, mock_config, mock_llm_adapter, mock_storage):
        ...
        tasks = [
            generator.generate('readme', f'/project{i}')
            for i in range(3)
        ]
