...
class PerformanceMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_latency_ms": (
                statistics.quantiles(self.operations, n=20)[18] * 1000
                if len(self.operations) > 20
                else 0
            ),
            "p99_latency_ms": (
                statistics.quantiles(self.operations, n=100)[98] * 1000
                if len(self.operations) > 100
                else 0
            ),
            "memory_used_mb": (
                self.memory_end - self.memory_start if self.memory_end and self.memory_start else 0
            ),
        }
    ...
    def print_report(self):
        ...
        if stats["ops_per_second"] >= target:
            print(
                f"✅ PASSED: Achieved {stats['ops_per_second']:,.0f} ops/sec (target: {target:,})"
            )
        else:
...
class TestStoragePerformance:
    ...
    def test_baseline_write_performance(self, in_memory_storage):
        ...
        assert (
            stats["ops_per_second"] > 1000
        ), f"Baseline write too slow: {stats['ops_per_second']:.0f} ops/sec"
    ...
    def test_encrypted_write_performance(self, storage_manager):
        ...
        stats = metrics.get_stats()
        assert (
            stats["ops_per_second"] > 100
        ), f"Encrypted write too slow: {stats['ops_per_second']:.0f} ops/sec"
    ...
    def test_encrypted_read_performance(self, storage_manager):
        ...
        assert (
            stats["ops_per_second"] > 100
        ), f"Encrypted read too slow: {stats['ops_per_second']:.0f} ops/sec"
    ...
    def test_query_performance(self, storage_manager):
        ...
        query_tests = [
            ...
            (
                "Search by Author",
                lambda i: storage.search_by_metadata({"author": f"author_{i % 10}"}),
            ),
...
def run_performance_suite():
    ...
    result = subprocess.run(
        ["python", "-m", "pytest", __file__, "-v", "-s"], capture_output=False, text=True
    )
