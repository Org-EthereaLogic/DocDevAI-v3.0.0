...
class PerformanceComparison:
    ...
    def run_write_benchmark(self, storage, name: str, start_id: int = 100000):
        ...
        def write_op():
            doc = Document(
                id=f"doc_{start_id + write_op.counter:06d}",
                content="x" * 100,
                type="test"
            )
    ...
    def run_bulk_benchmark(self, storage, name: str, start_id: int = 200000):
        ...
        if hasattr(storage, 'bulk_get'):
            ...
            return {
                'bulk_save_ops': bulk_save_ops,
                'bulk_get_ops': bulk_get_ops
            }
    ...
    def compare_implementations(self):
        ...
        try:
            ...
            orig_enc_read = self.run_read_benchmark(original_enc, doc_ids[:100], "Original (Encrypted)")
            opt_enc_read = self.run_read_benchmark(optimized_enc, doc_ids[:100], "Optimized (Encrypted)")
