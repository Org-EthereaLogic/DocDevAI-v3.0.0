...
ALLOWED_DEPENDENCY_FILES = {
    '.txt', '.toml', '.json', '.xml', '.csproj', '.mod', '.sum', '.lock', '.config'
}
...
class RateLimiter:
    ...
    def check(self, operation: str) -> bool:
        ...
        with self._lock:
            ...
            # Clean old requests
            self._requests[operation] = [
                t for t in self._requests[operation] if t > window_start
            ]
        ...
        try:
            ...
            if resolved.is_symlink():
                audit_logger.warning(
                    ...
                    extra={"security_event": "symlink_detected"}
                )
                return False
            ...
            audit_logger.error(
                ...
                extra={"security_event": "path_validation_error"}
            )
            return False
