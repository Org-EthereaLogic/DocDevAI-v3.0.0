...
class ParallelProcessor:
    ...
    def __init__(self, max_workers: int = DEFAULT_MAX_WORKERS):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="review_worker"
        )
        self._semaphore_map = {}

    async def process_parallel(
        self, tasks: List[Tuple[str, asyncio.Task]], max_concurrent: int = 6
    ) -> List[Tuple[str, Any]]:
    ...
    async def process_batch(
        self, items: List[Any], processor_func, batch_size: int = DEFAULT_BATCH_SIZE
    ) -> List[Any]:
        ...
        for i in range(0, len(items), batch_size):
            ...
            batch_results = await asyncio.gather(
                *[processor_func(item) for item in batch], return_exceptions=False
            )
