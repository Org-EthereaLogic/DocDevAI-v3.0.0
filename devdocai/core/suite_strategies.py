...
from .suite_types import (
    ConsistencyReport, ImpactAnalysis, DocumentGap, DependencyIssue,
    ImpactSeverity, ChangeType, EffortRange
)
...
class BasicConsistencyStrategy(ConsistencyStrategy):
    ...
    async def analyze(self, documents: List[Document]) -> ConsistencyReport:
        ...
        summary = f"Suite Consistency: {'Good' if consistency_score >= 0.8 else 'Needs Attention'} " \
    ...
    def _find_gaps(self, documents: List[Document]) -> List[DocumentGap]:
        ...
        for expected in expected_types:
            if expected not in existing_types:
                found = any(expected in doc_id.lower() or doc_id == expected 
    ...
    def _calculate_reference_integrity(
        ...
        total_refs = sum(len(re.findall(r'\[([^\]]+)\]\(#([^\)]+)\)', doc.content)) 
                        for doc in documents)
...
class BFSImpactStrategy(ImpactStrategy):
    ...
    async def analyze(
        ...
        effort_hours = self._estimate_effort(change_type, len(directly_affected), len(indirectly_affected))
