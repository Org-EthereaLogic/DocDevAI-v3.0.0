...
class ParallelImpactAnalysis:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for doc, impact in current_level.items():
                    ...
                    future = executor.submit(
                        self._process_document_impact, doc, depth, impact, max_depth
                    )
    ...
    def _process_document_impact(
        self, doc: str, depth: int, impact: float, max_depth: int
        ...
        return {"affected": affected, "scores": scores, "next_level": next_level}
...
class TrackingMatrix:
    ...
    def analyze_impact(self, doc_id: str, max_depth: int = None) -> ImpactResult:
        ...
        if max_depth is None:
            max_depth = (
                self.config.get("tracking_matrix", {})
                .get("impact_analysis", {})
                .get("max_depth", 5)
            )
