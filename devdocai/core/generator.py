...
class CryptoUtils:
    ...
    def encrypt(self, data: bytes) -> bytes:
        ...
        iv = os.urandom(12)
        cipher = Cipher(algorithms.AES(self.key[:32]), modes.GCM(iv), backend=default_backend())
    ...
    def decrypt(self, encrypted: bytes) -> bytes:
        ...
        cipher = Cipher(
            algorithms.AES(self.key[:32]), modes.GCM(iv, tag), backend=default_backend()
        )
...
class ResponseCache(BaseCache):
    ...
    def put(self, key: str, value: Any, **kwargs) -> None:
        ...
        with self.lock:
            ...
            threading.Thread(
                target=self._save_to_disk, args=(fingerprint, entry), daemon=True
            ).start()
...
class ContextCache(BaseCache):
    ...
    def put(self, key: str, value: Any, **kwargs) -> None:
        ...
        with self.lock:
            self.cache[key] = {"context": value, "timestamp": time.time()}
...
class SecurityValidator(BaseValidator):
    ...
    def validate(self, content: Any, **kwargs) -> ValidationResult:
        ...
        return ValidationResult(is_valid=score > 0.5, score=max(0, score), issues=issues)
...
class ContextBuilder:
    ...
    def extract_from_project(self, project_path: str) -> Dict[str, Any]:
        ...
        if not self.security_utils.validate_path(project_path):
            raise DocumentGenerationError("Invalid project path", error_type="security_violation")
    ...
    def _extract_file_context(self, project_dir: Path) -> Dict[str, Any]:
        ...
        context = {"total_files": 0, "file_types": defaultdict(int), "directories": []}
...
class PromptEngine:
    ...
    def construct_prompt(self, template: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise DocumentGenerationError(
                f"Prompt construction failed: {e}", error_type="prompt_construction"
            )
...
class DocumentGenerator:
    ...
    async def _generate_with_ai(self, template: Dict[str, Any], context: Dict[str, Any]) -> str:
        ...
        if not sections:
            ...
            prompt = self.prompt_engine.construct_prompt(
                template.get("prompt", "Generate documentation"), context
            )
            system_prompt = self.prompt_engine.construct_system_prompt(
        ...
        if independent_sections and len(independent_sections) > 1:
            ...
            for section in sections:
                ...
                if response and getattr(response, "content", None):
                    ...
                    context[f"section_{section.get('title', '').lower().replace(' ', '_')}"] = (
                        response.content
                    )
    ...
    async def _generate_sections_parallel(
        self, sections: List[Dict], context: Dict[str, Any], system_prompt: str
    ) -> List[str]:
    ...
    async def _validate_document(self, document: str, template: Dict[str, Any]) -> ValidationResult:
        ...
        return ValidationResult(is_valid=avg_score > 0.6, score=avg_score, issues=all_issues)
    ...
    async def generate_batch(
        self, requests: List[BatchRequest], max_concurrent: int = None
        ...
        for group in grouped:
            group_results = await asyncio.gather(*[generate_with_limit(req) for req in group])
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "memory_mode": (
                getattr(getattr(self.config, "system", None), "memory_mode", None)
