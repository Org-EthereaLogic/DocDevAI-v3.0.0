...
from ..core.storage import StorageManager, Document
from ..core.generator import DocumentGenerator
from ..core.tracking import TrackingMatrix, DependencyGraph, RelationshipType
...
class SuiteManagerFactory:
    ...
    def create(cls, **kwargs) -> 'SuiteManager':
        ...
        return SuiteManager(
            consistency_strategy=consistency,
            impact_strategy=impact,
            **kwargs
        )
...
class SuiteManager:
    ...
    def get_batch_size(self) -> int:
        ...
        mode = getattr(self.config, 'get_memory_mode', lambda: "balanced")() if self.config else "balanced"
        return {"minimal": 10, "balanced": 50, "performance": 100, "maximum": 500}.get(mode, 50)
    
    def get_cache_size(self) -> int:
        """Get cache size for memory mode."""
        mode = getattr(self.config, 'get_memory_mode', lambda: "balanced")() if self.config else "balanced"
        return {"minimal": 100, "balanced": 500, "performance": 1000, "maximum": 5000}.get(mode, 500)
    ...
    def _validate_refs(self, docs: List[Document], refs: Dict[str, List[str]]) -> Dict[str, List[str]]:
...
__all__ = [
    ...
    "SuiteConfig", "SuiteResult", "DocumentSuite",
    "ConsistencyReport", "ImpactAnalysis",
    "ImpactSeverity", "ChangeType",
    # Re-export from strategies
    "ConsistencyStrategy", "BasicConsistencyStrategy", "AdvancedConsistencyStrategy",
    "ImpactStrategy", "BFSImpactStrategy", "GraphImpactStrategy",
    # Re-export from security
    "RateLimiter", "ResourceMonitor", "InputValidator",
    # Exceptions
    "SuiteError", "ConsistencyError", "ImpactAnalysisError",
    "ValidationError", "SecurityError", "RateLimitError", "ResourceLimitError",
]