...
class AnalysisReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data["reviews"] = {
            k.value: v for k, v in self.reviews.items()
        }
        return data
...
class ReviewEngine:
    ...
    def __init__(
        ...
        self.storage = storage if storage is not None else (
            StorageManager(self.config) if self.config else None
        )
            ...
            try:
                ...
                for pii_type, pattern in pii_patterns.items():
                    if pattern and pii_type != "email":
                        from .review_types import PIIType, PIIPattern
                        self.pii_detector.add_pattern(
                            PIIType.CUSTOM,
                            pattern,
                            f"Custom {pii_type} pattern"
                        )
    ...
    async def _perform_analysis(
        ...
        content_chunks = (
            ...
            if len(document.content) > 50000 else None
        )
