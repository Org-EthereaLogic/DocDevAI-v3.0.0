...
class StorageManager:
    ...
    def save_document(self, document: Document) -> bool:
        ...
        try:
            ...
            cursor.execute(self._statements['insert_document'], (
                document.id,
                encrypted,
                iv,
                hmac_str,
            ...
            if self._encryption_enabled:
                ...
                try:
                    ...
                    if not hmac.compare_digest(hmac_str, expected_hmac):
                        ...
                        raise IntegrityError("HMAC verification failed", document_id=document_id,
                                             expected_hmac=hmac_str, actual_hmac=expected_hmac)
                    return cached
                finally:
                    cursor.close()
                    if not (hasattr(self, '_test_conn') and conn == self._test_conn) and not getattr(self._txn_state, 'active', False):
                        self._pool.return_connection(conn)
            else:
                return cached
    ...
    def update_document(self, document_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'content' in updates:
                ...
                cursor.execute(self._statements['update_document'], 
        ...
        try:
            ...
            for doc in documents:
                try:
                    ...
                    cursor.execute(self._statements['insert_document'], (
                        doc.id, encrypted, iv, hmac_str, doc.type,
                        doc.created_at, doc.updated_at, 1
                    ))
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "database_size": (Path(self.db_path).stat().st_size if self.db_path != ":memory:" else 0),
                "cache": cache_stats
            }
