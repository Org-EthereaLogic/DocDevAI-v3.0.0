...
class SecurityManager:
    ...
    def __init__(self, config: ConfigurationManager):
        ...
        self.rate_limit_per_minute = getattr(
            config.security, "rate_limit_per_minute", 240
        )  # 4 docs/sec
        ...
        try:
            ...
            if not str(resolved).startswith(str(allowed_dir)):
                self._log_security_event(
                    "PATH_TRAVERSAL", {"path": str(path), "resolved": str(resolved)}
                )
                return False
            ...
            for pattern in self.path_blacklist:
                if re.search(pattern, path_str):
                    self._log_security_event(
                        "PATH_BLACKLIST", {"path": path_str, "pattern": pattern}
                    )
                    return False
    ...
    def validate_batch_size(self, batch_size: int) -> bool:
        ...
        if batch_size > self.max_batch_size:
            ...
            self._log_security_event(
                "BATCH_SIZE_EXCEEDED", {"size": batch_size, "max": self.max_batch_size}
            )
            return False
        return True
    ...
    def check_rate_limit(self, user_id: str = "default") -> bool:
        ...
        with self.rate_limit_lock:
            ...
            self.request_timestamps[user_id] = [
                ts for ts in self.request_timestamps[user_id] if now - ts < 60
            ]
        ...
        for pattern, pii_type in self.pii_patterns:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    self._log_security_event(
                        "PII_DETECTED", {"type": pii_type, "count": len(matches)}
                    )
    ...
    def encrypt_cache_content(self, content: bytes) -> bytes:
        ...
        # Create cipher
        cipher = Cipher(algorithms.AES(self.cache_key), modes.GCM(iv), backend=default_backend())
    ...
    def decrypt_cache_content(self, encrypted: bytes) -> bytes:
        ...
        # Create cipher
        cipher = Cipher(
            algorithms.AES(self.cache_key), modes.GCM(iv, tag), backend=default_backend()
        )
    ...
    def _log_security_event(self, event_type: str, details: Dict[str, Any]):
        ...
        with self.audit_lock:
            ...
            event["signature"] = hmac.new(
                self.cache_key, event_str.encode(), hashlib.sha256
            ).hexdigest()
    ...
    def validate_resource_usage(
        self, memory_mb: float, cpu_percent: float, concurrent: int, memory_mode: str
    ) -> bool:
        ...
        if violations:
            self.security_metrics["resource_violations"] += 1
            self._log_security_event(
                "RESOURCE_QUOTA_EXCEEDED", {"violations": violations, "memory_mode": memory_mode}
            )
            return False
...
class ResponseCache:
    ...
    def __init__(
        self, config: ConfigurationManager, security_manager: Optional["SecurityManager"] = None
    ):
        ...
        # Cache statistics
        self.stats = {"hits": 0, "misses": 0, "evictions": 0, "similarity_matches": 0}
            ...
            if fingerprint in self.l1_cache:
                ...
                if self._is_valid_secure(entry, fingerprint, user_id):
                    ...
                    self.security_manager._log_security_event(
                        "CACHE_INTEGRITY_FAILURE", {"fingerprint": fingerprint, "user_id": user_id}
                    )
    ...
    def _is_valid_secure(self, entry: CacheEntry, fingerprint: str, user_id: str) -> bool:
        ...
        if entry.user_id != user_id and entry.user_id != "default":
            self.security_manager._log_security_event(
                "CACHE_ISOLATION_VIOLATION", {"expected_user": user_id, "entry_user": entry.user_id}
            )
            return False
    ...
    def _save_to_disk(self, fingerprint: str, entry: CacheEntry):
        ...
        try:
            ...
            self.security_manager._log_security_event(
                "CACHE_SAVE_FAILURE", {"fingerprint": fingerprint, "error": str(e)}
            )
    ...
    def _load_from_disk(self, fingerprint: str) -> Optional[CacheEntry]:
        ...
        try:
                ...
                if self._is_valid(entry):
                    ...
                    if entry.signature:
                        if not self.security_manager.verify_cache_signature(
                            ...
                            self.security_manager._log_security_event(
                                "CACHE_LOAD_INTEGRITY_FAILURE", {"fingerprint": fingerprint}
                            )
                            return None
                    return entry
            ...
            self.security_manager._log_security_event(
                "CACHE_LOAD_FAILURE", {"fingerprint": fingerprint, "error": str(e)}
            )
        return None
...
class BatchProcessor:
    ...
    def __init__(
        ...
        concurrency_map = {"baseline": 10, "standard": 50, "enhanced": 200, "performance": 1000}
    ...
    async def process_batch(
        self, requests: List[BatchRequest], user_id: str = "default"
        ...
        if len(requests) > quota["concurrent"]:
            self.security_manager._log_security_event(
                "BATCH_CONCURRENT_EXCEEDED",
                {"requested": len(requests), "max_allowed": quota["concurrent"]},
            )
        ...
        logger.info(
            f"Batch processed {len(requests)} documents in {elapsed:.2f}s ({docs_per_second:.1f} docs/s)"
        )
            ...
            if op_id in self.start_times:
                ...
                if len(self.metrics[operation]) < self.max_metrics_per_operation:
                    ...
                    if self.security_manager:
                        self.security_manager._log_security_event(
                            "METRICS_ROTATION",
                            {"operation": operation, "count": len(self.metrics[operation])},
                        )
    ...
    def _create_default_templates(self):
        ...
        default_templates = {
            ...
            "api_doc": {
                ...
                "context_requirements": ["project_name", "classes", "functions"],
                "quality_criteria": {
                    "min_length": 1000,
            ...
            "changelog": {
                ...
                "context_requirements": ["project_name", "version"],
                "quality_criteria": {"min_length": 200, "required_sections": ["header", "version"]},
            },
        }
...
class ContextBuilder:
    ...
    def _extract_python_context(self, project_dir: Path) -> Dict[str, Any]:
        ...
        context = {"modules": [], "classes": [], "functions": [], "main_modules": []}
                    ...
                    if isinstance(node, ast.ClassDef):
                        ...
                        if not node.name.startswith("_"):
                            context["functions"].append(
                                self.security_manager.sanitize_input(node.name)
                            )
    ...
    def _extract_file_context(self, project_dir: Path) -> Dict[str, Any]:
        ...
        context = {"file_count": 0, "total_size": 0, "file_types": {}}
...
class PromptEngine:
    ...
    def format_template(
        self, template: str, context: Dict[str, Any], use_defaults: bool = True
    ) -> str:
...
class DocumentGenerator:
    ...
    def __init__(
        ...
        worker_counts = {"baseline": 4, "standard": 8, "enhanced": 16, "performance": 32}
        ...
        logger.info(
            f"DocumentGenerator initialized with {self.memory_mode} memory mode, {self.max_workers} workers"
        )
        ...
        try:
            ...
            if use_cache:
                cached_response = self.response_cache.get(
                    self.prompt_engine.construct_system_prompt(document_type), context
                )
            ...
            if cached_response:
                ...
                if use_cache and not cache_hit:
                    self.response_cache.put(
                        self.prompt_engine.construct_system_prompt(document_type), context, content
                    )
            ...
            while not validation_result.is_valid and retry_on_failure and retry_count < max_retries:
                logger.info(
                    f"Validation failed, retrying... (attempt {retry_count + 1}/{max_retries})"
                )
            ...
            logger.info(
                f"Successfully generated {document_type} document with score {validation_result.score}"
            )
    ...
    async def _generate_sequential_sections(
    ...
    async def _generate_parallel_sections(
        self, template: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
    ...
    async def _extract_context_optimized(self, project_path: str) -> Dict[str, Any]:
        ...
        if self.process_pool:
            ...
            context = await loop.run_in_executor(
                self.process_pool, self.context_builder.extract_from_project, project_path
            )
        else:
    ...
    async def _generate_parallel_optimized(
        self, template: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
    ...
    async def _generate_single_cached(
        self, prompt: str, context: Dict[str, Any], user_id: str = "default"
    ) -> str:
        ...
        self.response_cache.put(
            prompt, context, response.content, response.tokens_used, response.cost, user_id
        )
    ...
    def clear_caches(self):
        ...
        self.security_manager._log_security_event(
            "CACHE_CLEAR", {"timestamp": datetime.now().isoformat()}
        )
        self.response_cache = ResponseCache(self.config, self.security_manager)
        self.context_cache = ContextCache(self.config)
...
async def generate_document(
    document_type: str, project_path: str, config: Optional[ConfigurationManager] = None, **kwargs
) -> Dict[str, Any]:
