    ...
    def __post_init__(self):
        ...
        if len(self.documents) > MAX_SUITE_SIZE:
            raise ResourceLimitError(
                f"Suite size {len(self.documents)} exceeds maximum {MAX_SUITE_SIZE}"
            )
        ...
        for doc in self.documents:
            ...
            if "content" in doc:
                ...
                if content_size > MAX_DOCUMENT_SIZE:
                    raise ResourceLimitError("Document size exceeds maximum")

        if self.cross_references:
            total_refs = sum(len(refs) for refs in self.cross_references.values())
            if total_refs > MAX_CROSS_REFS:
                raise ResourceLimitError("Cross-references exceed maximum")
