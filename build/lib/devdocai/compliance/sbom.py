...
from .sbom_core import Ed25519Signer, LicenseDetector, SBOMValidator, VulnerabilityScanner
...
from .sbom_strategies import FormatBuilderFactory, PackageScannerFactory
from .sbom_types import SBOM, Package, SBOMError, SBOMFormat, SBOMSignature
            ...
            with ThreadPoolExecutor(max_workers=6) as executor:
                ...
                for future in as_completed(future_to_scanner):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"{scanner_name} found {len(result)} packages in {scan_duration:.2f}s"
                        )
...
class SBOMGenerator:
        ...
        with self._lock:
            ...
            try:
                ...
                self._perf_monitor.record_operation(
                    "license_detection_batch", license_duration, size=licenses_detected
                )
                ...
                if scan_vulnerabilities:
                    ...
                    self._perf_monitor.record_operation(
                        "vulnerability_scan_batch", vuln_duration, size=len(vulnerabilities)
                    )
                ...
                self._perf_monitor.record_operation(
                    "sbom_generation_total", elapsed, size=len(sbom.packages)
                )
    ...
    def export(
        self, sbom: SBOM, output_path: Path, pretty: bool = True, use_streaming: bool = True
    ) -> None:
        """
        ...
        with self._lock:
            ...
            try:
                ...
                if should_stream:
                    ...
                    try:
                        loop.run_until_complete(
                            StreamingExporter.stream_json_export(json_data, output_path)
                        )
                    finally:
                        loop.close()
                else:
                ...
                self._perf_monitor.record_operation("sbom_export", export_duration, size=file_size)
