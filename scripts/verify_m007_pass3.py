...
async def test_security_features():
    ...
    try:
        ...
        if accuracy >= 0.90 and total_found >= 4 and false_positives == 0:
            results["pii_detection"] = f"✅ Passed - {accuracy:.1%} accuracy, {total_found} found, 0 false positives"
        elif accuracy >= 0.85:
            results["pii_detection"] = f"⚠️ Partial - {accuracy:.1%} accuracy (target: 95%), {false_positives} false positives"
        else:
            results["pii_detection"] = f"❌ Failed - {accuracy:.1%} accuracy, {false_positives} false positives"
    except Exception as e:
